...
class TemplateType extends Type {
	...
	private $backuppath = '/app/Resources/views/backup/'; 
	...
	protected function getPath($filename) {
		return PRJ_REF.$this->basepath.$filename;
	}
	
	protected function getRealPath($filename) {
		return PRJ_DIR.$this->basepath.$filename;
	}
	
	public function getBackupPath($filename) {
		return PRJ_REF.$this->backuppath.$filename;
	}
	
	public function getBackupRealPath($filename) {
		return PRJ_DIR.$this->backuppath.$filename;
	}
	...
	public function getSQLValue($name = '') {
		...
		$filename = $this->get('util')->post($name.'_oldvalue');
		$date_stamp = date('Y_m_d_H_i_s');
		if ($filename && $this->get('util')->post($name.'_delete')) {
			$this->backupTemplate($filename, $filename.$date_stamp.'.bak');	
			$filename = '';
		} elseif ($filename && $this->get('util')->post($name.'_version', true, 0)) {
			$sql = "SELECT * FROM template_version WHERE id= :id ";
			$stmt = $this->get('connection')->prepare($sql);
			$stmt->bindValue('id', $this->get('util')->post($name.'_version', true, 0));
			$stmt->execute();
			$template = $stmt->fetch();
			if ($template) {
				$this->backupTemplate($filename, $filename.$date_stamp.'.bak');
		...
		} elseif ($this->get('util')->post($name.'_cre')) {
			$filename = trim($this->get('util')->post($name));
			if ($filename != '') {
